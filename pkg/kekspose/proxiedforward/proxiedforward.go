...
/*
This file is based on https://github.com/kubernetes/client-go/blob/master/tools/portforward/portforward.go
Compared to the original file, this one injects my own logic for forwarding the ports while proxying them.
*/
...
*/
func parsePorts(ports []string) ([]ProxiedPort, error) {
	var forwards []ProxiedPort
	for _, portString := range ports {
		...
		forwards = append(forwards, ProxiedPort{uint16(localPort), uint16(remotePort)})
	}
...
func (pf *ProxiedForwarder) ForwardPorts() error {
	defer pf.Close()
...
func (pf *ProxiedForwarder) forward() error {
	var err error
...
func (pf *ProxiedForwarder) listenOnPort(port *ProxiedPort) error {
	var errors []error
...
func (pf *ProxiedForwarder) getListener(protocol string, hostname string, port *ProxiedPort) (net.Listener, error) {
...
// the background.
func (pf *ProxiedForwarder) waitForConnection(listener net.Listener, port ProxiedPort) {
	for {
		select {
		case <-pf.streamConn.CloseChan():
			return
...
func (pf *ProxiedForwarder) nextRequestID() int {
...
func (pf *ProxiedForwarder) Close() {
	// stop all listeners
...
func (pf *ProxiedForwarder) GetPorts() ([]ProxiedPort, error) {
	if pf.Ready == nil {
